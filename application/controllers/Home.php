        ...
        if($course_details['course_type'] == 'general'){
            ...
            if ($sections->num_rows() > 0) {
                ...
                if ($lesson_id == "") {
                    ...
                    $cur_lesson_data = $section_id = $this->db->get_where('lesson', array('id' => $lesson_id))->row();
                    $cur_course_id = $cur_lesson_data->course_id;
                    $section_id = $cur_lesson_data->section_id;
                    while(1) {
                        $row = $this->db->get_where('lesson', array('id' => $section_id))->row();
                        if (empty($row)) { break; }
                        if ($row->lesson_type != "term" && $row->lesson_type != "subterm") { break; }
                        if ($row->lesson_type == "term" && $row->course_id != $cur_course_id) { break; }
