...
class Admin extends CI_Controller {
    ...
    public function upload_summernote_audio() {
        $path_parts = pathinfo($_FILES['audio']['name']);
        $audio_path = $path_parts['filename'].'_'.time().'.'.$path_parts['extension'];

        $uploaddir = 'uploads/summernote/';
        $uploadfile = $uploaddir . str_replace(' ', '-', $audio_path);
    ...
    public function upload_summernote_image() {
        ...
        $uploaddir = 'uploads/summernote/';
        $uploadfile = $uploaddir . str_replace(' ', '-', $image_path);
    ...
    public function enrol_course($param1 = "") {
        ...
        $courses = $this->crud_model->get_courses_by_search_string('')->result_array();
        ...
        $page_data['active_course'] = $course_id;
        $page_data['courses'] = $courses;
        $page_data['page_name'] = 'enrol_course';
        $page_data['enrol_history'] = $this->crud_model->enrol_history_by_course($course_id);
        $page_data['page_title'] = get_phrase('enrol_by_course');
        $this->load->view('backend/index', $page_data);
    }

    public function enrol_course_export() {
        if ($this->session->userdata('admin_login') != true) {
            redirect(site_url('login'), 'refresh');
        }
        ...
        if ($this->input->get('down_course_id')) {
            $course_id = intval($this->input->get('down_course_id'));
        }

        $enrol_history = $this->crud_model->enrol_history_by_course($course_id);

        $this->load->helper('download');
        $data = array();
        $data['title'] = 'Export Excel Sheet';
        $fileName = 'Course Enrollment -'.time();
        ...
        $sheet->setCellValue('D1', 'Enrollment Date');

        $rowCount = 2;
        foreach($enrol_history->result_array() as $enrol) {
            $user_data = $this->db->get_where('users', array('id' => $enrol['user_id']))->row_array();
            $sheet->setCellValue('A' . $rowCount, $rowCount - 1);
            $sheet->setCellValue('B' . $rowCount, $user_data['first_name'] . ' ' . $user_data['last_name']);
            $sheet->setCellValue('C' . $rowCount, $user_data['email']);
            $sheet->setCellValue('D' . $rowCount, date('D, d-M-Y', $enrol['date_added']));
            $rowCount++;
        }
    ...
    public function get_courses() {
        ...
        if(!empty($courses)) {
            foreach ($courses as $key => $row) {
                ...
                $lessons = $this->crud_model->get_lessons('lesson_count_in_course', $row->id);
                ...
                $duplicate_course_url = "confirm_modal('".site_url('admin/course_actions/duplicate/'.$row->id)."')";
                $action = '
                ...
                <li><a class="dropdown-item" href="javascript::" onclick="'.$duplicate_course_url.'">'.get_phrase("duplicate_course").'</a></li>
                '.$section_and_lesson_menu.'
                ...
                $nestedData['thumbnail'] = '<img width="80" height="53" style="" src="'.$this->crud_model->get_course_thumbnail_url($row->id, 'course_thumbnail').'"/>';
    ...
    public function course_actions($param1 = "", $param2 = "") {
        ...
        }elseif ($param1 == "duplicate") {
            $this->crud_model->duplicate_course($param2);
    ...
    public function blog($param1 = "", $param2 = "") {
        ...
        if ($param1 == 'category') {
            $page_data['languages'] = $this->crud_model->get_all_languages();
            $page_data['categories'] = $this->crud_model->get_categories();
            $page_data['page_name'] = 'blog_category_manage';
            $page_data['page_title'] = get_phrase('manage_blog_category');
            $this->load->view('backend/index', $page_data);
        }
        if ($param1 == 'manage') {
            $page_data['languages'] = $this->crud_model->get_all_languages();
    ...
    public function blog_action($param1 = "add") {
        if ($this->session->userdata('admin_login') != true) {
            redirect(site_url('login'), 'refresh');
        }
        ...
        if ($param1 == 'add') {
            ...
            redirect(site_url('admin/blog/manage'), 'refresh');
        }
    }
    ...
    public function blog_category_update() {
        ...
        $this->crud_model->blog_category_update();
        redirect(site_url('admin/blog/category'), 'refresh');
    }
    ...
    public function migration($type) {
        $apiEnrollData = [];
        if ($type == 'evol') {

            $old_user_array = [];
            $users = $this->user_model->get_all_user()->result();
            foreach ($users as $user) {
                $old_user_array[$user->email] = $user->id;
            }

            $newUserData = [];
            $existing_evol = [];

            $prev_evol_ids = $this->db->get_where('settings', ['key' => 'evol_migration'])->result();
            $prev_evol_ids = count($prev_evol_ids) > 0 ? explode(",", $prev_evol_ids[0]->value) : [];
            $prev_evol_exist = count($prev_evol_ids) > 0 ? true : false;
            foreach($prev_evol_ids as $id) {
                $existing_evol[intval($id)] = 1;
            }

            $apiEnrollData = $this->evolcampus->getEnrollments($existing_evol);
            foreach ($apiEnrollData as $key => $userData) {
                $usr_id = $old_user_array[$userData['email']]; 
                if (!$usr_id) {
                    $data = [];
                    $data['first_name'] = html_escape($userData['name']);
                    $data['last_name'] = html_escape($userData['lastname']);
                    $data['email'] = html_escape($userData['email']);
                    $data['password'] = sha1(html_escape('TbV#0p$wd'));
                    $data['social_links'] = '{"facebook":"","twitter":"","linkedin":""}';
                    $data['biography'] = '';
                    $data['role_id'] = 2;
                    $data['date_added'] = strtotime(date("Y-m-d H:i:s"));
                    $data['wishlist'] = json_encode(array());
                    $data['status'] = 1;
                    $data['image'] = html_escape($userData['photo']);
                    $data['paypal_keys'] = '[{"production_client_id":null,"production_secret_key":null}]';
                    $data['stripe_keys'] = '[{"public_live_key":"","secret_live_key":""}]';
                    $data['watch_history'] = '[]';                    
                    array_push($newUserData, $data);
                }
            }

            $this->db->insert_batch('users', $newUserData);
            ...
            foreach ($users as $user) {
                ...
                $watch_history_array[$user->id]  = json_decode($user->watch_history, true);
            }

            $lessons = $this->crud_model->get_lessons("total_lesson_count")->result_array();
            $courses = array();
            foreach ($lessons as $lesson) {
                if (isset($courses[$lesson['course_id']])) {
                    array_push($courses[$lesson['course_id']], $lesson['id']);    
                } else {
                    $courses[$lesson['course_id']] = array();
                    $courses[$lesson['course_id']][] = $lesson['id'];
                }
            }
            
            foreach ($apiEnrollData as $key => $userData) {
                $usr_id = $user_array[$userData['email']]; 
                
                array_push($userWatchData, $usr_id);

                foreach ($userData['enroll'] as $key1 => $enroll) {
                    if ($enroll['courseid']) {
                        array_push($prev_evol_ids, intval($enroll['enrollmentid']));
                        $passed_lesson_cnt = intval(count($courses[$enroll['courseid']]) * floatval($enroll['completedpercent']) / 100.0);
                        for ($i = 0 ; $i < $passed_lesson_cnt; $i++) {
                            array_push($watch_history_array[$usr_id], [
                                'lesson_id' => $courses[$enroll['courseid']][$i],
                                'progress' => 1,
                            ]);
                        }
                        array_push($enrolData, [
                            'user_id' => $usr_id,
                            'course_id' => $enroll['courseid'],
                            'date_added' => strtotime($enroll['begin']),
                        ]);
                    }
                }
            }
            for ($i = 0; $i < count($userWatchData); $i++) {
                $tmp = [
                    'id' => $userWatchData[$i],
                    'watch_history' => json_encode($watch_history_array[$userWatchData[$i]]),
                ];
                $userWatchData[$i] = $tmp;
            }
    
            $this->db->update_batch('users', $userWatchData, 'id');
            $this->db->insert_batch('enrol', $enrolData);

            if ($prev_evol_exist) {
                $this->db->where('key', 'evol_migration');
                $this->db->update('settings', ['value' => implode(",", $prev_evol_ids)]);
            } else {
                $this->db->insert('settings', [
                    'key' => 'evol_migration',
                    'value' => implode(",", $prev_evol_ids),
                ]);
            }
            $page_data['res'] = $apiEnrollData;
            $page_data['page_name'] = 'migration_evol';            
        }
        elseif ($type == 'woo') {
            $page_data['res'] = [];
            $page_data['page_name'] = 'migration_woo';
        }
    ...
    public function lessons($course_id = "", $param1 = "", $param2 = "") {
        ...
        elseif ($param1 == 'move') {
            $this->crud_model->move_lesson($param2);
            $this->session->set_flashdata('flash_message', get_phrase('lesson_has_been_moved_successfully'));
            redirect('admin/course_form/course_edit/'.$course_id);
        }
    ...
    // Manage Terms
    public function terms($course_id = "", $action = "", $lesson_id = "") {
        if ($this->session->userdata('admin_login') != true) {
            redirect(site_url('login'), 'refresh');
        }

        if ($action == 'add') {
            $this->crud_model->add_term($course_id);
            $this->session->set_flashdata('flash_message', get_phrase('term_has_been_added_successfully'));
        }
        elseif ($action == 'edit') {
            $this->crud_model->edit_term($lesson_id);
            $this->session->set_flashdata('flash_message', get_phrase('term_has_been_updated_successfully'));
        }
        elseif ($action == 'delete') {
            $this->crud_model->delete_term($lesson_id);
            $this->session->set_flashdata('flash_message', get_phrase('term_has_been_deleted_successfully'));
        }
        elseif ($action == 'move') {
            $this->crud_model->move_term($lesson_id);
            $this->session->set_flashdata('flash_message', get_phrase('term_has_been_deleted_successfully'));
        }
    ...
    // Manage SubTerms
    public function subterms($course_id = "", $action = "", $lesson_id = "") {
        if ($this->session->userdata('admin_login') != true) {
            redirect(site_url('login'), 'refresh');
        }

        if ($action == 'add') {
            $this->crud_model->add_subterm($course_id);
            $this->session->set_flashdata('flash_message', get_phrase('term_has_been_added_successfully'));
        }
        elseif ($action == 'edit') {
            $this->crud_model->edit_term($lesson_id);
        ...
        elseif ($action == 'delete') {
            $this->crud_model->delete_subterm($lesson_id);
            $this->session->set_flashdata('flash_message', get_phrase('term_has_been_deleted_successfully'));
        }
        elseif ($action == 'move') {
            $this->crud_model->move_subterm($lesson_id);
            $this->session->set_flashdata('flash_message', get_phrase('term_has_been_deleted_successfully'));
        }
        redirect(site_url('admin/course_form/course_edit/'.$course_id));
    }
