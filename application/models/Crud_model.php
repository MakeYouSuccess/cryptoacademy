...
class Crud_model extends CI_Model
{
    ...
    public function enrol_history_by_course($course_id)
    {
        $this->db->order_by('date_added', 'desc');
    ...
    public function get_blog_category($all = '') {
        if ($all == 'all') return $this->db->get('blog_category');
        return $this->db->where('active', 1)->get('blog_category');
    }

    public function blog_category_update() {
        if (isset($_POST['categories'])) {
            $updated_categories = $_POST['categories'];
            $blog_category = $this->crud_model->get_blog_category('all')->result_array();
            $cat_names = array_map(function ($ctg) {
                return $ctg['name'];
            }, $blog_category);
            $this->db->update('blog_category', ['active' => 0]);
            foreach ($updated_categories as $category) {
                if ($category && !in_array($category, $cat_names)) {
                    $this->db->insert('blog_category', [
                        'name' => $category,
                        'active' => 1,
                    ]);
                } else {
                    $this->db->where(['name' => $category])->update('blog_category', [
                        'active' => 1,
                    ]);
                }
            }
    ...
    public function add_blog() {
        ...
        $data['category'] = implode(",", $this->input->post('blog_categories'));
        $data['content'] = html_escape($this->input->post('content'));
        $this->db->insert('blog', $data);
    }

    public function get_lessons($type = "", $id = "", $parentType = "")
    {
        $this->db->order_by("order", "asc");
        if ($type == "course") {
            return $this->db->get_where('lesson', array('course_id' => $id));
        } elseif ($type == "section") {
            return $this->db->get_where('lesson', array('section_id' => $id, 'parent_type' => $parentType));
        } elseif ($type == "lesson") {
            return $this->db->get_where('lesson', array('id' => $id));
        } elseif ($type == "lesson_count_in_course") {
            $this->db->where('course_id', $id);
            $this->db->where_not_in('lesson_type', ['term', 'subterm']);
            return $this->db->get('lesson');    
        }  elseif ($type == "total_lesson_count") {
            $this->db->where_not_in('lesson_type', ['term', 'subterm']);
            return $this->db->get('lesson');
        } else {
            return $this->db->get('lesson');
    ...
    public function duplicate_course($src_course_id)
    {
        $src_course_details = $this->get_course_by_id($src_course_id)->row_array();
        $data = $src_course_details;
        ...
        $sections = $this->db->get_where('section', array('course_id' => $src_course_id))->result_array();
        foreach ($sections as $oldSection) {
            $newSection = $oldSection;
            $newSection['course_id'] = $course_id;
            unset($newSection['id']);
            $this->db->insert('section', $newSection);
            $newSectionId = $this->db->insert_id();

            $terms = $this->db->get_where('lesson', array('course_id' => $src_course_id, 'section_id' => $oldSection['id']))->result_array();
            foreach ($terms as $oldTerm) {
                $newTerm = $oldTerm;
                $newTerm['course_id'] = $course_id;
                $newTerm['section_id'] = $newSectionId;
                unset($newTerm['id']);
                $this->db->insert('lesson', $newTerm);
                $newTermId = $this->db->insert_id();
                
                if($newTerm['lesson_type'] == 'term') {
                    $subterms = $this->db->get_where('lesson', array('course_id' => $src_course_id, 'section_id' => $oldTerm['id']))->result_array();
                    foreach ($subterms as $oldSubterm) {
                        $newSubTerm = $oldSubterm;
                        $newSubTerm['course_id'] = $course_id;
                        $newSubTerm['section_id'] = $newTermId;
                        unset($newSubTerm['id']);
                        $this->db->insert('lesson', $newSubTerm);
                        $newSubTermId = $this->db->insert_id();
                        
                        if($newSubTerm['lesson_type'] == 'subterm') {
                            $lessons = $this->db->get_where('lesson', array('course_id' => $src_course_id, 'section_id' => $oldSubterm['id']))->result_array();
                            $newLessons = array();
                            foreach ($lessons as $oldLesson) {
                                $newLesson = $oldLesson;
                                $newLesson['course_id'] = $course_id;
                                $newLesson['section_id'] = $newSubTermId;
                                unset($newLesson['id']);
                                $newLessons[] = $newLesson;
                            }
                            $this->db->insert_batch('lesson', $newLessons);
                        }
                    }    
                }
            }
        ...
        $course_media_files = themeConfiguration(get_frontend_settings('theme'), 'course_media_files');
        foreach ($course_media_files as $course_media => $size) {
            $srcFile = 'uploads/thumbnails/course_thumbnails/' . $course_media . '_' . get_frontend_settings('theme') . '_' . $src_course_id . '.jpg';
            $destFile = 'uploads/thumbnails/course_thumbnails/' . $course_media . '_' . get_frontend_settings('theme') . '_' . $course_id . '.jpg';
    ...
    public function update_course($course_id, $type = "")
    {
        ...
        $data['short_description'] = html_escape($this->input->post('short_description'));
        $data['description'] = $this->input->post('description');
        $data['for_whom'] = $this->input->post('for_whom');
        $data['outcomes'] = $outcomes;
        $data['language'] = $this->input->post('language_made_in');
        $data['sub_category_id'] = $this->input->post('sub_category_id');
        $category_details = $this->get_category_details_by_id($this->input->post('sub_category_id'))->row_array();
        ...
        /* --------- temary pdf upload -----------*/
        $path_parts = pathinfo($_FILES['temary_url']['name']);
        if ($path_parts['basename'] != "") {
            move_uploaded_file($_FILES['temary_url']['tmp_name'], 'uploads/thumbnails/course_thumbnails/course_tempary_' . $course_id . '.' . $path_parts['extension']);
            $data['temary_url'] = 'uploads/thumbnails/course_thumbnails/course_tempary_' . $course_id . '.' . $path_parts['extension'];
        }
        /* --------- temary thumbnail upload -----------*/
        $path_parts = pathinfo($_FILES['course_temary_thumbnail']['name']);
        if ($path_parts['basename'] != "") {
            move_uploaded_file($_FILES['course_temary_thumbnail']['tmp_name'], 'uploads/thumbnails/course_thumbnails/course_tempary_thumbnail' . $course_id . '.' . $path_parts['extension']);
            $data['temary_thumbnail'] = 'uploads/thumbnails/course_thumbnails/course_tempary_thumbnail' . $course_id . '.' . $path_parts['extension'];
        }
    ...
    public function add_lesson()
    {
        ...
        $term_id = html_escape($this->input->post('term_id'));
        $subterm_id = html_escape($this->input->post('subterm_id'));
        $data['section_id'] = $subterm_id != "-1" ? $subterm_id : ($term_id != "-1" ? $term_id : $section_id);
        $data['parent_type'] = $subterm_id != "-1" ? 'les' : ($term_id != "-1" ? 'les' : "");
        ...
        $data['date_added'] = strtotime(date('D, d-M-Y'));
        $data['summary'] = html_entity_decode($this->input->post('summary'));
    ...
    public function move_lesson($lesson_id)
    {
        $section_id = html_escape($this->input->post('section_id'));
        $term_id = html_escape($this->input->post('term_id'));
        $subterm_id = html_escape($this->input->post('subterm_id'));
    ...
    public function add_term($course_id) {
        $data['course_id'] = $course_id;
        $data['title'] = html_escape($this->input->post('title'));
        $data['section_id'] = html_escape($this->input->post('section_id'));
        $data['lesson_type'] = 'term';
        $data['date_added'] = strtotime(date('D, d-M-Y'));

        $this->db->insert('lesson', $data);
    }

    public function move_term($lesson_id) {
        $data['section_id'] = html_escape($this->input->post('section_id'));
    ...
    public function edit_term($lesson_id) {
        $data['title'] = html_escape($this->input->post('title'));
    ...
    public function delete_term($lesson_id) {
        $this->db->where('id', $lesson_id);
        $term = $this->db->get('lesson')->result_array()[0];

        $this->db->order_by("order", "asc");
        $subterms = $this->db->get_where('lesson', array('section_id' => $lesson_id, 'course_id' => $term['course_id'], 'lesson_type' => 'subterm'))->result_array();
        foreach($subterms as $subterm) {
            $this->delete_subterm($subterm['id']);
        }
        $this->db->where('id', $lesson_id);
        $this->db->delete('lesson');
    }

    public function add_subterm($course_id) {
        $data['course_id'] = $course_id;
        $data['title'] = html_escape($this->input->post('title'));
        $data['section_id'] = html_escape($this->input->post('term_id'));
        $data['lesson_type'] = 'subterm';
        $data['parent_type'] = 'les';
        $data['date_added'] = strtotime(date('D, d-M-Y'));

        $this->db->insert('lesson', $data);
    }

    public function move_subterm($lesson_id) {
        $section_id = html_escape($this->input->post('section_id'));
        $term_id = html_escape($this->input->post('term_id'));
        $data['section_id'] = $term_id == '' ? $section_id : $term_id;
        $data['lesson_type'] = $term_id == '' ? 'term' : 'subterm';
        $data['parent_type'] = $term_id == '' ? '' : 'les';

        $this->db->where('id', $lesson_id);
        $this->db->update('lesson', $data);
    }

    public function delete_subterm($lesson_id) {
        $this->db->where('id', $lesson_id);
        $subterm = $this->db->get('lesson')->result_array()[0];

        $this->db->where('section_id', $lesson_id);
        $this->db->where('course_id', $subterm['course_id']);
        $this->db->where('parent_type', 'les');
        $this->db->delete('lesson');
        $this->db->where('id', $lesson_id);
        $this->db->delete('lesson');
    }
    ...
    public function add_quiz_questions($quiz_id)
    {
        ...
        parse_str($rawData, $inputData);
        $question_type = $inputData['question_type'];
        if ($question_type == 'mcq') {
            $response = $this->add_multiple_choice_question($quiz_id, $inputData);
            return $response;
        }
    }
    ...
    public function update_quiz_questions($question_id)
    {
        ...
        $question_type = $inputData['question_type'];
        if ($question_type == 'mcq') {
            $response = $this->update_multiple_choice_question($question_id, $inputData);
            return $response;
        }
    }
    // multiple_choice_question crud functions
    function add_multiple_choice_question($quiz_id, $inputData)
    {
        if (sizeof($inputData['options']) != $inputData['number_of_options']) {
            return false;
        }
        ...
        if (sizeof($inputData['correct_answers']) == 0) {
            $correct_answers = [""];
        } else {
            $correct_answers = $inputData['correct_answers'];
        }
        $data['quiz_id']            = $quiz_id;
        $data['title']              = rawurldecode($inputData['title']);
        $data['number_of_options']  = html_escape($inputData['number_of_options']);
        $data['type']               = 'multiple_choice';
        $data['options']            = json_encode($inputData['options']);
        $data['correct_answers']    = json_encode($correct_answers);
        $this->db->insert('question', $data);
        return true;
    }
    // update multiple choice question
    function update_multiple_choice_question($question_id, $inputData)
    {
        if (sizeof($inputData['options']) != $inputData['number_of_options']) {
            return false;
        }
