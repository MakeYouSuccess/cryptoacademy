...
class Api_model extends CI_Model
{
	...
	// Get Terms
	public function terms_get($section_id) {
        $this->db->order_by("order", "asc");
        return $this->db->get_where('lesson', array('section_id' => $section_id, 'lesson_type' => 'term'))->result_array();
	}
	
	// Get SubTerms
	public function subterms_get($section_id) {
		$this->db->order_by("order", "asc");
        return $this->db->get_where('lesson', array('section_id' => $section_id, 'lesson_type' => 'subterm'))->result_array();
	}
	...
	public function my_courses_get($user_id = "")
	{
		...
		foreach ($my_courses as $key => $my_course) {
			if (isset($my_course['id']) && $my_course['id'] > 0) {
				...
				$my_courses[$key]['total_number_of_lessons'] = $this->crud_model->get_lessons('lesson_count_in_course', $my_course['id'])->num_rows();
	...
	public function lessons_from_parent_id_get($parent_id = "", $parent_type = "", $user_id = "")
	{
		$this->db->order_by("order", "asc");
		return $this->db->get_where('lesson', array('section_id' => $parent_id, "parent_type" => $parent_type))->result_array();
	}

	//get sections info
	public function sections_info_get($course_id = "", $user_id = "")
	{
	...
	public function get_completed_number_of_lesson($user_id = "", $type = "", $id = "")
	{
		...
		if ($type == 'section') {
			...
			$lessons = $this->crud_model->get_lessons('lesson_count_in_course', $id)->result_array();
		}
		foreach ($lessons as $key => $lesson) {
	...
	public function course_details_by_id_get($user_id = "", $course_id = "")
	{
		...
		foreach ($response as $key => $resp) {
			...
			$response[$key]['includes'] = array(
				...
				$this->crud_model->get_lessons('lesson_count_in_course', $course_id)->num_rows() . ' ' . get_phrase('lessons'),
	...
	private function course_completion_data($course_id = "", $user_id = "")
	{
		...
		$response['number_of_lessons'] = $this->crud_model->get_lessons('lesson_count_in_course', $course_id)->num_rows();
