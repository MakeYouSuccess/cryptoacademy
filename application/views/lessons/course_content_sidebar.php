											...
											foreach ($lessons as $key => $lesson): ?>
												<?php if ($lesson['lesson_type'] != 'term'): ?>
													...
													<tr style="width: 100%; padding: 5px 0px;background-color: <?php if ($lesson_id == $lesson['id'])echo '#E6F2F5'; else echo '#fff';?>;">
														<td style="text-align: left; padding:7px 10px; border-left: 1px dotted #097e97">
															<?php
															$lesson_count++;
															...
															<div class="lesson_duration">
																...
																<?php elseif($lesson['lesson_type'] == 'editable'): ?>
																	<i class="fas fa-chalkboard-teacher"></i> <?php echo get_phrase('content'); ?>
												...
												<?php else: 
													...
													<tr style="width: 100%; background-color: #fff;">
														<td style="text-align: left; border-left: 1px dotted #097e97">
															<button class="btn btn-link text-left w-100" type="button" style="color: #535a66; background-color: #eee; border:none; white-space: normal;">
																<h6 style="color: #444549; font-size: 13px; margin-bottom: 0px;">
																	<?php echo get_phrase('term').' '.($term_count).': '.$lesson['title'];?>
																	<span style="float: left; margin-right: 5px; font-weight: 100;">
																		<i class="fa fa-minus"></i>
																	</span>
																</h6>
															</button>
															<div style="margin-left: 15px; width: calc(100% - 15px);">
																<table style="width: 100%">
																	<?php
																		// SUBTERM and LESSONS
																		...
																		<?php if ($subterm['lesson_type'] != 'subterm'): ?>
																			<?php
																				if(isset($bundle_id) && $bundle_id > 0):
																					$subterm_url = site_url('addons/course_bundles/lesson/'.rawurlencode(slugify($course_details['title'])).'/'.$bundle_id.'/'.$course_id.'/'.$subterm['id']);
																				else:
																					$subterm_url = site_url('home/lesson/'.slugify($course_details['title']).'/'.$course_id.'/'.$subterm['id']);
																				endif;
																				?>

																				<tr style="width: 100%; padding: 5px 0px;background-color: <?php if ($lesson_id == $subterm['id'])echo '#E6F2F5'; else echo '#fff';?>;">
																					<td style="text-align: left; padding:7px 10px; border-left: 1px dotted #097e97">
																						<?php
																						$lesson_count++;
																						$lesson_progress = lesson_progress($subterm['id']);
																						?>
																						<div class="form-group">
																							<input type="checkbox" id="<?php echo $subterm['id']; ?>" onchange="markThisLessonAsCompleted(this.id);" value = 1 <?php if($lesson_progress == 1):?> checked <?php endif; ?>>
																							<label for="<?php echo $subterm['id']; ?>"></label>
																						</div>

																						<a href="<?= $subterm_url; ?>" id = "<?php echo $subterm['id']; ?>" style="color: #444549;font-size: 14px;font-weight: 400;">
																							<?php echo $lesson_count; ?>:
																							<?php if ($subterm['lesson_type'] != 'other'):?>
																								<?php echo $subterm['title']; ?>
																							<?php else: ?>
																								<?php echo $subterm['title']; ?>
																								<!-- <i class="fa fa-paperclip"></i> -->
																							<?php endif; ?>
																						...
																						<div class="lesson_duration">
																							<?php if ($subterm['lesson_type'] == 'video' || $subterm['lesson_type'] == '' || $subterm['lesson_type'] == NULL): ?>
																								<?php //echo $subterm['duration']; ?>
																								<i class="far fa-play-circle"></i>
																								<?php echo readable_time_for_humans($subterm['duration']); ?>
																							<?php elseif($subterm['lesson_type'] == 'quiz'): ?>
																								<i class="far fa-question-circle"></i> <?php echo get_phrase('quiz'); ?>
																							<?php else: ?>
																								<?php if ($subterm['attachment_type'] == 'iframe'): ?>
																									<i class="fas fa-code"></i> <?php echo get_phrase('external_source'); ?>
																								<?php else: ?>
																									<?php $tmp = explode('.', $subterm['attachment']);
																									$fileExtension = strtolower(end($tmp));?>
																									<?php if ($fileExtension == 'jpg' || $fileExtension == 'jpeg' || $fileExtension == 'png' || $fileExtension == 'bmp' || $fileExtension == 'svg'): ?>
																										<i class="fas fa-camera-retro"></i> <?php echo get_phrase('attachment'); ?>
																									<?php elseif($fileExtension == 'pdf'): ?>
																		...
																		<?php else:
																			...
																			<tr style="width: 100%; background-color: #fff;">
																				<td style="text-align: left; border-left: 1px dotted #097e97">
																					<button class="btn btn-link text-left w-100" type="button" style="color: #535a66; background-color: #eee; border: none; white-space: normal;">
																						<h6 style="color: #444549; font-size: 13px; margin-bottom: 0px;">
																							<?php echo get_phrase('subterm').' '.($subterm_count).':'.$subterm['title'];?>
																							<span style="float: left; margin-right: 5px; font-weight: 100;">
																					...
																					<div style="margin-left: 15px; width: calc(100% - 15px);">
																						<table style="width: 100%">
																							<?php
																							// Subterm_lessons
																							$subterm_lessons = $this->crud_model->get_lessons('section', $subterm['id'], 'les')->result_array();
																							foreach ($subterm_lessons as $index => $subterm_lesson):?>
																							<?php
																								if(isset($bundle_id) && $bundle_id > 0):
																									$subterm_url = site_url('addons/course_bundles/lesson/'.rawurlencode(slugify($course_details['title'])).'/'.$bundle_id.'/'.$course_id.'/'.$subterm_lesson['id']);
																								else:
																									$subterm_url = site_url('home/lesson/'.slugify($course_details['title']).'/'.$course_id.'/'.$subterm_lesson['id']);
																								endif;
																								?>

																								<tr style="width: 100%; padding: 5px 0px;background-color: <?php if ($lesson_id == $subterm_lesson['id'])echo '#E6F2F5'; else echo '#fff';?>;">
																									<td style="text-align: left; padding:7px 10px; border-left: 1px dotted #097e97">
																										<?php
																										$lesson_count++;
																										$lesson_progress = lesson_progress($subterm_lesson['id']);
																										?>
																										<div class="form-group">
																											<input type="checkbox" id="<?php echo $subterm_lesson['id']; ?>" onchange="markThisLessonAsCompleted(this.id);" value = 1 <?php if($lesson_progress == 1):?> checked <?php endif; ?>>
																											<label for="<?php echo $subterm_lesson['id']; ?>"></label>
																										</div>

																										<a href="<?= $subterm_url; ?>" id = "<?php echo $subterm_lesson['id']; ?>" style="color: #444549;font-size: 14px;font-weight: 400;">
																											<?php echo $lesson_count; ?>:
																											<?php if ($subterm_lesson['lesson_type'] != 'other'):?>
																												<?php echo $subterm_lesson['title']; ?>
																											<?php else: ?>
																												<?php echo $subterm_lesson['title']; ?>
																												<!-- <i class="fa fa-paperclip"></i> -->
																											<?php endif; ?>
																										...
																										<div class="lesson_duration">
																											<?php if ($subterm_lesson['lesson_type'] == 'video' || $subterm_lesson['lesson_type'] == '' || $subterm_lesson['lesson_type'] == NULL): ?>
																												<?php //echo $subterm_lesson['duration']; ?>
																												<i class="far fa-play-circle"></i>
																												<?php echo readable_time_for_humans($subterm_lesson['duration']); ?>
																											<?php elseif($subterm_lesson['lesson_type'] == 'quiz'): ?>
																												<i class="far fa-question-circle"></i> <?php echo get_phrase('quiz'); ?>
																											<?php else: ?>
																												<?php if ($subterm_lesson['attachment_type'] == 'iframe'): ?>
																													<i class="fas fa-code"></i> <?php echo get_phrase('external_source'); ?>
																												<?php else: ?>
																													<?php $tmp = explode('.', $subterm_lesson['attachment']);
																													$fileExtension = strtolower(end($tmp));?>
																													<?php if ($fileExtension == 'jpg' || $fileExtension == 'jpeg' || $fileExtension == 'png' || $fileExtension == 'bmp' || $fileExtension == 'svg'): ?>
																														<i class="fas fa-camera-retro"></i> <?php echo get_phrase('attachment'); ?>
																													<?php elseif($fileExtension == 'pdf'): ?>
