...
<div class="mt-3">
    ...
    <div class="custom-control custom-radio">
        <input type="radio" id="editable" name="lesson_type" class="custom-control-input" value="editable" <?php if($selected_lesson == 'editable') echo 'checked'; ?>>
        <label class="custom-control-label" for="editable"><?php echo get_phrase('editable'); ?></label>
    </div>
...
<script type="text/javascript">
    function showLessonAddModal() {
        ...
        if(course_id > 0){
            ...
            if ($("input[name=lesson_type]:checked").val() == 'editable' ||
                $("input[name=lesson_type]:checked").val() == 'vimeo' ||
                $("input[name=lesson_type]:checked").val() == 'youtube')
