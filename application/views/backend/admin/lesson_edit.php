...
<div class="alert alert-info" role="alert">
    ...
    <strong>
        ...
        }elseif($lesson_details['lesson_type'] == 'editable'){
            echo get_phrase('editable');
        }
        ?>.
    </strong>
</div>
...
<form id="form_lesson" action="<?php echo site_url('admin/lessons/'.$param3.'/edit'.'/'.$param2); ?>" method="post" enctype="multipart/form-data">
    ...
    <!-- <div class="form-group">
        ...
        <select class="form-control select2" data-toggle="select2" name="section_id" id="section_id" required type="hidden">
    ...
    <?php if ($lesson_details['lesson_type'] != 'editable' &&
                $lesson_details['lesson_type'] != 'video'): ?>
        <div class="form-group">
    ...
    <?php else:?>
        <div class="form-group">
            <label><?php echo get_phrase('content'); ?></label>
            <textarea id="summary" name="summary" class="form-control"><?php echo $lesson_details['summary']; ?></textarea>
        </div>
        <?php if ($lesson_details['lesson_type'] == 'editable'):?>
            <input type="hidden" name="lesson_type" value="editable-editable">
            <?php endif;?>
    <?php endif;?>

    <div class="text-center">
        <button class = "btn btn-success" type="submit" name="button" onclick="onSubmit(event)"><?php echo get_phrase('update_lesson'); ?></button>
    </div>
</form>
...
function onSubmit(event) {
    ...
    $("#summary").val(htmlEntities(summary));
    event.preventDefault();
    $("#form_lesson").submit();
}
$(document).ready(function() {
    console.log('<?=$lesson_details['lesson_type']?>');
    <?php if ($lesson_details['lesson_type'] == 'editable' ||
                $lesson_details['lesson_type'] == 'video'): ?>
        initSummerNoteEx(['#summary'], "<?php echo site_url('admin/upload_summernote'); ?>");
    <?php endif;?>
